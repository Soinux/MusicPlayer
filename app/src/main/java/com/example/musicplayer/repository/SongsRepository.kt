...
import com.example.musicplayer.extension.getInt
import com.example.musicplayer.extension.getLong
import com.example.musicplayer.extension.getString
import com.example.musicplayer.model.Song
import com.example.musicplayer.utils.FilePathUtlis
...
//        val albumArtist = cursor.getString(AudioColumns.ALBUM_ARTIST)
    ...
    private fun getSongsFromStorage(): ArrayList<Song> {
        ...
        Log.d("music", "getting songs from ${FilePathUtlis.getMusicsUri()}")
        if (cursor == null) Log.d("SongsRepository", "getSongsFromStorage null cursor")
        if (cursor != null) Log.d("SongsRepository", "getSongsFromStorage count ${cursor.count}")
        if (cursor != null && cursor.count != 0) {
            while(cursor.moveToNext()) {
                Log.d("SongsRepository", "${createSongFromCursor(cursor)}")
